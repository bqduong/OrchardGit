    ...
    public class UserOrdersServices : IUserOrdersServices
    {
        ...
        private void LoadOrders(UserOrdersPart part, OrdersViewModel viewModel)
        {
            Exception exeption = this._webStoreClientServices.UsingClient(
                c =>
                {
                    OrderList orderList = c.OrderClient.GetOrders(this._webStoreClientServices.CurrentUserName, viewModel.State.SortExpression.Value, viewModel.State.SortDirection.Value, part.EnablePaging && part.PageSize.HasValue ? part.PageSize.Value : Int32.MaxValue, viewModel.State.Page.Value - 1, false, false);
