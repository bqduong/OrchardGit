...
namespace Magelia.WebStore.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        private AddressViewModel Initialize(AddressViewModel viewModel)
        {
            ...
            {
                this._webStoreServices.UsingClient(c => viewModel.Regions = c.StoreClient.GetRegions(viewModel.CountryId.Value, this._webStoreServices.CurrentCultureId).Select(r => new SelectListItem { Text = r.Name, Value = r.RegionId.ToString() }));
        ...
        private ActionResult BuildCustomerAddressRecap(Nullable<Guid> addressId)
        {
            ...
            {
                this._webStoreServices.UsingClient(
                    c => address = c.CustomerClient.GetAddresses(this._webStoreServices.CurrentUserName, this._webStoreServices.CurrentCultureId)
        ...
        public CheckoutController(IWebStoreServices webStoreServices, IOrchardServices orchardServices, IEnumerable<IBasketEventHandler> basketEventHandlers, IShapeFactory shapeFactory)
        {
        ...
        public ActionResult SaveAsOrder(PaymentViewModel viewModel)
        {
            ...
            {
                Order order = null;
                SaveAsOrderResult result = SaveAsOrderResult.UnknownError;

                Exception exception = this._webStoreServices.UsingClient(c => order = c.BasketClient.SaveAsOrder(this._webStoreServices.CurrentUserName, this._webStoreServices.BasketName, viewModel.BasketHash, this.Request.UserHostAddress, null, out result));
                ...
                {
                    this._basketEventHandlers.Trigger(h => h.SavedAsOrder(order));
                ...
                else
                {
                    switch (result)
                    {
                        ...
                        case SaveAsOrderResult.BasketNotFound:
                            ...
                            break;
                        case SaveAsOrderResult.UnknownError:
                            this.ModelState.AddModelError("UnexpectedError", this._localizer("An unexpected error has occured, please try again later").ToString());
                            break;
